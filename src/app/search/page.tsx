    ...
    <div className="container mx-auto fade-in px-4 py-8">
      ...
      <section className="fade-in">
        ...
        <div className="mt-4 space-y-6">
          ...
          {results.length > 0 ? (
            results.map((result) => (
              <div key={result.document.id} className="bg-card p-6 rounded-lg border border-border shadow-md hover:shadow-lg transition-shadow relative">
                 <div className="absolute top-4 right-4 bg-primary text-primary-foreground px-2 py-1 rounded-md text-xs font-semibold">
                   Relevansi: {result.relevancePercentage.toFixed(2)}% {/* Displaying percentage score */}
                 </div>
                <div className="mt-4 text-foreground leading-relaxed" dangerouslySetInnerHTML={{ __html: result.highlightedContent }} />
              </div>
            ))
          ) : (
            !searching && noResults && (
